...
def _score_cands(tokenizer, model, source, cands):
    with torch.no_grad():
        input_ids = tokenizer(source.replace('<mask>', '<extra_id_0>'), return_tensors='pt').input_ids
        labels_ten = tokenizer('<extra_id_0> %s <extra_id_1>' % cands[-1], return_tensors='pt').input_ids
        logits = model(input_ids=input_ids, labels=labels_ten).logits # (B=1, L, V)
        labels = tokenizer(['<extra_id_0> %s <extra_id_1>' % cand for cand in cands[:-1]], return_tensors='pt').input_ids  # (B, L)
...
def main():
    ...
    args.output_path = f'data/numersense/inference/inference_mix_gold_5_{args.model_type.split("/")[-1]}.{args.input_path.split("/")[-1]}'
