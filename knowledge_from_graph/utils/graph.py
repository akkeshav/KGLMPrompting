...
def concepts2adj(node_ids, cpnet_graph_path, cpnet_vocab_path):
    global concept2id, id2concept, relation2id, id2relation
...
def concepts_to_adj_matrices_2hop_all_pair(data):
    ...
    qmask = arange < len(qc_ids)
    amask = (arange >= len(qc_ids)) & (arange < (len(qc_ids) + len(ac_ids)))
    adj, concepts, adj_original = concepts2adj(schema_graph)
    return adj, concepts, qmask, amask, adj_original
...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part3(data):
    ...
    amask = (arange >= len(qc_ids)) & (arange < (len(qc_ids) + len(ac_ids)))
    adj, concepts, adj_original = concepts2adj(schema_graph)
    return {'adj': adj, 'concepts': concepts, 'qmask': qmask, 'amask': amask, 'cid2score': cid2score, 'adj_o': adj_original}
